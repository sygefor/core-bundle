...
namespace Sygefor\Bundle\CoreBundle\Entity\Session\Term;
...
use Sygefor\Bundle\CoreBundle\Form\PlaceType;
...
class Place extends AbstractTerm implements VocabularyInterface
{
    ...
    {
        $addressParts = array();
        $addressParts[] = $this->getAddress();

        $cityParts = array();
        $cityParts[] = $this->getPostal();
        $cityParts[] = $this->getCity();

        $implodedParts = implode(' ', $cityParts);
        if ($implodedParts !== ' ') {
            $addressParts[] = $implodedParts;
        }
